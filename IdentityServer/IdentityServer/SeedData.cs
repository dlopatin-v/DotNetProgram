...
public class SeedData
{
    ...
    private static void EnsureRoles(IServiceScope scope)
    {
        ...
        if (manager is null)
        {
            ...
            result = roleMgr.AddClaimAsync(manager, 
                new Claim(JwtClaimTypes.Name, "Read")
            ).Result;
            if (!result.Succeeded)
            {
                throw new Exception(result.Errors.First().Description);
            ...
            result = roleMgr.AddClaimAsync(manager,
                new Claim(JwtClaimTypes.Name, "Create")
            ).Result;
            if (!result.Succeeded)
            {
                throw new Exception(result.Errors.First().Description);
            ...
            result = roleMgr.AddClaimAsync(manager,
                new Claim(JwtClaimTypes.Name, "Update")
            ).Result;
            if (!result.Succeeded)
            {
                throw new Exception(result.Errors.First().Description);
            ...
            result = roleMgr.AddClaimAsync(manager,
                new Claim(JwtClaimTypes.Name, "Delete")
            ).Result;
            if (!result.Succeeded)
            {
                throw new Exception(result.Errors.First().Description);
        ...
        var buyer = roleMgr.FindByNameAsync("Buyer").Result;
        if (buyer is null)
        {
            buyer = new IdentityRole
            {
                Name = "Buyer"
            };
            var result = roleMgr.CreateAsync(buyer).Result;
            if (!result.Succeeded)
            {
                throw new Exception(result.Errors.First().Description);
            }
            result = roleMgr.AddClaimAsync(buyer,
                new Claim(JwtClaimTypes.Name, "Read")
            ).Result;
            if (!result.Succeeded)
            {
